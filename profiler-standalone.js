...
// Fixed: Proper timestamp-query feature detection and pass instrumentation
  ...
  async function hookWebGPU() {
    ...
    navigator.gpu.requestAdapter = async function(options) {
      ...
      adapter.requestDevice = async function(descriptor) {
        ...
        // CRITICAL: Add timestamp-query to requiredFeatures BEFORE device creation
        ...
        device.createCommandEncoder = function(desc) {
          ...
          encoder.beginComputePass = function(passDesc) {
            ...
            // CRITICAL FIX: Only inject timestamps if we have space AND timestamps are enabled
            if (profilerData.hasTimestamps && profilerData.timestampCount + 2 <= 512) {
              timestampStart = profilerData.timestampCount;
              timestampEnd = profilerData.timestampCount + 1;
              profilerData.timestampCount += 2;

              const newDesc = { 
                ...passDesc, 
                timestampWrites: {
                  querySet: profilerData.querySet,
                  beginningOfPassWriteIndex: timestampStart,
                  endOfPassWriteIndex: timestampEnd
                }
              };
              pass = origBeginComputePass(newDesc);
              pass.__tsStart = timestampStart;
              pass.__tsEnd = timestampEnd;
              addLog(`Pass created with timestamps [${timestampStart}, ${timestampEnd}]`);
            } else {
              pass = origBeginComputePass(passDesc);
              pass.__tsStart = -1;
              pass.__tsEnd = -1;
              if (profilerData.hasTimestamps) {
                addLog('Timestamp slots full, falling back to CPU timing', 'warning');
              }
            }

            pass.__dispatches = [];
            pass.__passIndex = profilerData.dispatches.length;
            ...
            pass.dispatchWorkgroups = function(x, y, z) {
              ...
              const dispatchRecord = {
                ...
                x, y, z,
                gpuTime: null,
                timestampStart: this.__tsStart,
                timestampEnd: this.__tsEnd,
                passIndex: this.__passIndex,
              ...
              origDispatch(x, y, z);
              dispatchRecord.cpuEnd = performance.now();
              
              // CPU fallback timing
              if (!profilerData.hasTimestamps || this.__tsStart === -1) {
                dispatchRecord.gpuTime = (dispatchRecord.cpuEnd - dispatchRecord.cpuStart) * 1000;
                dispatchRecord.timingSource = 'cpu-fallback';
              }
              
              // Log timing info to console
              ...
              console.log(`[WebSight] Dispatch #${dispatchRecord.index} (${pipeline?.label || 'unknown'}):`, {
                ...
                timingSource: dispatchRecord.timingSource || 'pending'
              });
        ...
        // Hook Queue Submit for Timestamp Resolution
        ...
        device.queue.submit = function(cmds) {
          ...
          if (profilerData.hasTimestamps && profilerData.timestampCount > 0 && !profilerData.readBuffer.__mapped) {
            ...
            device.queue.onSubmittedWorkDone().then(() => {
              ...
              // Group dispatches by pass
              const passesByIndex = {};
              profilerData.dispatches.forEach(d => {
                if (d.timestampStart >= 0 && d.timestampStart < times.length) {
                  if (!passesByIndex[d.passIndex]) {
                    passesByIndex[d.passIndex] = [];
                  }
                  passesByIndex[d.passIndex].push(d);
                }
              });

              // Process each pass
              Object.values(passesByIndex).forEach(passDispatches => {
                if (passDispatches.length === 0) return;
                
                const firstDispatch = passDispatches[0];
                const start = Number(times[firstDispatch.timestampStart]);
                const end = Number(times[firstDispatch.timestampEnd]);
                const passTime = (end - start) / 1000; // ns to μs
                
                // Divide pass time equally among dispatches
                const timePerDispatch = passTime / passDispatches.length;
                
                passDispatches.forEach(d => {
                  d.gpuTime = timePerDispatch;
                  d.timingSource = 'gpu_timestamp';

                  // Console log GPU timing results
                  ...
                  // Update Kernel Stats
                  if (d.kernelId && profilerData.kernels[d.kernelId]) {
