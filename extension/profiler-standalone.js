// WebGPU Compute Profiler - Modular & Production Ready
...
(function() {
  ...
  class TimingHelper {
    ...
    reset(numKernels) {
      ...
      if (this.#canTimestamp) {
        ...
        try {
          // Push error scope to catch GPU-level allocation failures
          ...
          // Check for errors asynchronously, but don't block construction
          this.#device.popErrorScope().then(error => {
            if (error) {
              console.warn(`[WebSight] QuerySet creation failed at GPU level: ${error.message}`);
              this.#canTimestamp = false;
              this.#querySet = null;
            }
          });
        } catch (e) {
          // If QuerySet creation fails, disable timing for this helper
          this.#canTimestamp = false;
    ...
    // Compatibility methods for WebSight's existing code
    getStats() {
      return {
  ...
  const profilerData = {
    ...
    // Encoder lifetime tracking
    activeEncoders: new WeakMap(), // encoder -> { dispatches: [], startTime: number }
    config: {
      broadcastEnabled: true,
      broadcastDebounceMs: 5000, // Changed from 100ms to 5000ms for lower overhead
      normalizeTimeUnit: 'us', // 'ns', 'us', or 'ms'
      
      // Logging control - set to true for debugging
      verboseLogging: false, // Enable all console logs for debugging
      
      // Performance mode - MINIMAL OVERHEAD
      minimalOverhead: false, // When true, disables GPU timing and reduces broadcasts
      
      // Advanced analysis features - DISABLED by default for performance
      // Enable when needed via WebSight.configure() or specific analysis calls
      enableMemoryLeakDetection: false,  // Only tracks when explicitly called
      enableWorkgroupAnalysis: false,    // Only analyzes when explicitly called
      enableShaderAnalysis: false,       // Only analyzes when explicitly called
      captureStacks: false,              // Very expensive - only for detailed leak debugging
      
      // Thresholds
      memoryLeakThresholdMs: 10000, // Consider resource leaked after 10s
      memoryWarningThresholdMB: 100 // Warn when total memory > 100MB
    }
  };

  // =============================================================================
  // MODULE 7: MEMORY LEAK DETECTOR
  // =============================================================================
  // Tracks resource creation/destruction to detect memory leaks
  
  class MemoryLeakDetector {
    constructor() {
      this.resources = new Map(); // id -> { type, size, createdAt, stack, destroyed }
      this.nextId = 0;
      this.leakThreshold = 10000; // 10 seconds without destruction = potential leak
      this.sizeThreshold = 100 * 1024 * 1024; // 100MB total = warning
      this.autoCheckInterval = null; // Only run when enabled
    ...
    trackResource(resource, type, size) {
      ...
      // Capture creation stack (expensive, only in debug mode)
      let stack = '';
      ...
      // Store ID on resource for later lookup
  ...
  // =============================================================================
  // MODULE 8: WORKGROUP OCCUPANCY ANALYZER
  // =============================================================================
  // Analyzes workgroup sizes and provides optimization recommendations
  ...
  class WorkgroupOccupancyAnalyzer {
    ...
    setDeviceLimits(limits) {
      ...
      // Estimate optimal parameters based on typical GPU architectures
      this.optimal = {
        // Most GPUs have warps/wavefronts of 32 or 64
        simdWidth: 64, // Conservative estimate
        
        // Typical shared memory per workgroup
        sharedMemoryPerWorkgroup: 32 * 1024, // 32KB
        
        // Typical max threads per workgroup
        maxThreadsPerWorkgroup: limits.maxComputeInvocationsPerWorkgroup || 256,
        
        // Recommended workgroup sizes for different patterns
        recommendations: {
          '1D': [256, 128, 64], // Prefer 256 for 1D
          '2D': [[16, 16], [8, 8], [32, 8]], // Common 2D sizes
          '3D': [[8, 8, 8], [4, 4, 4]] // 3D rarely used
        }
      };
    }
    ...
    analyzeDispatch(dispatch) {
      ...
      // Issue 2: Workgroup too small (low occupancy)
      ...
      // Issue 4: Unbalanced dimensions (e.g., [256, 1, 1] for 2D problem)
      ...
      // Issue 5: Non-power-of-2 dimensions
  ...
  // =============================================================================
  // MODULE 9: SHADER COMPLEXITY ANALYZER
  // =============================================================================
  // Analyzes WGSL shader code for complexity and optimization opportunities
  
  class ShaderComplexityAnalyzer {
    constructor() {
      this.analyses = new Map(); // shaderId -> analysis
      
      // Instruction costs (relative to simple ALU ops)
    ...
    analyzeShader(shaderId, wgslCode) {
      ...
      // Issue 1: Detect divergent branches (if statements in tight loops)
      ...
      // Issue 2: Expensive math operations
      ...
      // Issue 3: Register pressure (too many variables)
      ...
      // Issue 4: Uncoalesced memory access patterns
      ...
      // Issue 5: Redundant barrier synchronizations
      ...
      // Issue 6: Atomic operations (slow)
  ...
  // =============================================================================
  // MODULE 3: Broadcast & Communication
  // =============================================================================
  ...
  function broadcastData() {
    // Don't broadcast from UI window - it only receives data
  ...
  function normalizeTime(timeNs) {
    ...
    switch (unit) {
      ...
      default: return timeNs / 1000; // Default to µs
    }
  }
  ...
  async function hookWebGPU() {
    ...
    // Track multiple adapters and devices for multi-GPU support
    ...
    navigator.gpu.requestAdapter = async function(options) {
      ...
      adapter.requestDevice = async function(descriptor) {
        try {
          ...
          // CRITICAL: Check disable flag BEFORE setting profilerData.timingMode
          ...
          // Track this device with metadata
          const deviceInfo = {
            device,
            adapter,
            hasTimestampQuery,
          ...
          device.addEventListener('uncapturederror', (event) => {
            if (event.error.message && event.error.message.includes('Cannot allocate sample buffer')) {
              console.error('[WebSight] ⚠️ GPU QuerySet allocation failed!');
              console.log('[WebSight] Your GPU has reached its QuerySet limit.');
              console.log('[WebSight] 💡 Solution: Add this BEFORE loading profiler-standalone.js:');
          ...
          device.createShaderModule = function(desc) {
            ...
            // Don't analyze automatically - wait for user to call getShaderAnalysis()
          ...
          if (!window.__webSightGlobalTimingResults) {
            ...
            window.__webSightMaxTimingResults = 10000; // Prevent memory leak in long-running apps
          }
          
          // CRITICAL: Reusable TimingHelper pool to avoid QuerySet exhaustion
          ...
          const getTimingHelper = (device) => {
              ...
              try {
                ...
                device.popErrorScope().then(error => {
                  if (error) {
                    ...
                    console.error('[WebSight] ⚠️ GPU QuerySet creation failed:', error.message);
                ...
                // Check if we hit hard GPU limit
                if (e.message.includes('Cannot allocate') || e.message.includes('sample buffer') || e.message.includes('QuerySet')) {
                  pool.limitReached = true;
                  console.warn(`[WebSight] ⚠️ GPU QuerySet limit reached at ${pool.helpers.length} helpers.`);
                  ...
                  // If we have at least some helpers, continue by waiting
                  if (pool.helpers.length === 0) {
                    ...
                    // Auto-disable GPU timing for this device
                    profilerData.timingMode = 'cpu-only';
                    return null;
                  }
                  ...
                  // Return null - caller will handle graceful degradation
                  return null;
                } else {
                  console.warn('[WebSight] Cannot create TimingHelper:', e.message);
                  // Complete failure - disable timing for this device
                  if (pool.helpers.length === 0) {
                    pool.failed = true;
            ...
            // Pool is full and no helpers available - return null (graceful degradation)
            // Don't reuse helpers that are still in "wait for result" state
              ...
              pass.dispatchWorkgroups = function(x, y, z) {
                ...
                if (profilerData.config.enableWorkgroupAnalysis) {
                  ...
                  if (analysis && analysis.score < 70) {
                    ...
                    console.warn(`[WebSight] ⚠️ Suboptimal workgroup config for "${pipeline.label}" (Score: ${analysis.score})`);
          ...
          // CRITICAL: Expose timing data in MULTIPLE ways for universal compatibility
          // 
          // Method 1: For primitive.mjs framework (existing)
          // Method 2: Direct access via window.__webSightGlobalTimingResults
          // Method 3: Attach timing to command buffers (universal pattern)
          // Method 4: Events for real-time timing notifications
          
          if (!window.__WebSightTimingHelper) {
            window.__webSightTimingHelper_executionId = 0;
            
            // Method 1: Compatible with primitive.mjs's BasePrimitive.__timingHelper
  ...
  if (typeof window !== 'undefined') {
    window.WebSight = {
      ...
      // Quick presets for benchmarking
      benchmarkMode: () => {
        profilerData.config.minimalOverhead = true;
        profilerData.timingMode = 'cpu';
        profilerData.config.broadcastEnabled = false; // Disable broadcasts entirely
      ...
      normalMode: () => {
        ...
        profilerData.config.broadcastDebounceMs = 1000;
        console.log('[WebSight] 👁️  NORMAL MODE: GPU timing enabled, broadcasts active');
      },
